...
def launch_ui():
    ...
    /* ── Apply variables to Gradio ── */
    .gradio-container {
        --background-fill-primary: var(--bg-color) !important;
        --background-fill-secondary: var(--panel-color) !important;
        --block-background-fill: var(--panel-color) !important;
        --block-border-color: var(--card-color) !important;
        --border-color-primary: var(--card-color) !important;
        --border-color-accent: var(--accent-color) !important;
        --body-text-color: var(--text-color) !important;
        --body-text-color-subdued: var(--subdued-text) !important;
        --block-label-text-color: var(--subdued-text) !important;
        --block-title-text-color: var(--text-color) !important;
        ...
        --button-secondary-background-fill: var(--card-color) !important;
        --button-secondary-background-fill-hover: var(--subdued-text) !important;
        --button-secondary-text-color: var(--text-color) !important;
        
        --button-cancel-background-fill: var(--danger-color) !important;
        --button-cancel-background-fill-hover: var(--danger-hover) !important;
        
        --input-background-fill: var(--panel-color) !important;
        background: var(--bg-color) !important;
    }

    .tabs > .tab-nav button { color: var(--subdued-text) !important; }
    .tabs > .tab-nav button.selected { color: var(--accent-color) !important; border-color: var(--accent-color) !important; }
    ...
    .sign-cell:hover { border-color: var(--accent-color); }
    .sign-label { font-size: 1.1rem; font-weight: 700; color: var(--text-color); margin-top: 4px; }
    ...
    SIGNS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "signs")
    ALL_SIGNS = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ") + list("0123456789")

    _HOVER_JS = """
    function() {
        // Automatically click buttons with the 'sign-label-btn' class on hover
        document.addEventListener('mouseover', (e) => {
            if (e.target && e.target.classList && e.target.classList.contains('sign-label-btn')) {
                e.target.click();
            }
        });
    }
    ...
    with gr.Blocks(title="AI Sign Language → Text Converter") as demo:

        # ── HEADER ──────────────────────────────────────────────────────
        gr.Markdown("## AI Sign Language → Text Converter")
        ...
        with gr.Tabs():
                    ...
                    with gr.Tab("🎥 Live Webcam"):
                        ...
                        with gr.Row():
                            ...
                            with gr.Column(scale=2):
                                out_live = gr.Label(
                                    num_top_classes=5,
                                    label="🔤 Detected Letter & Confidence",
                                )
                                status = gr.Textbox(
                                    label="Status",
                                    interactive=False,
                                    value="Show your hand in the frame…",
                                )
                            ...
                            with gr.Column(scale=2):
                                detected_text = gr.Textbox(
                                    ...
                                    interactive=False,
                                    lines=4,
                                    placeholder="Confirmed letters appear here…",
                                )
                                with gr.Row():
                                    space_btn     = gr.Button("⎵ Space",     variant="secondary")
                                    backspace_btn = gr.Button("⌫ Backspace", variant="secondary")
                                clear_btn = gr.Button("🗑 Clear", variant="stop")

                        # ── Button logic ─────────────────────────────
                        def _clear():
                            return "", "", {}
                        clear_btn.click(fn=_clear, inputs=None, outputs=[detected_text, text_state, hold_state])

                        def _space(txt): s = txt + " "; return s, s
                        space_btn.click(fn=_space, inputs=[text_state], outputs=[detected_text, text_state])

                        def _backspace(txt): s = txt[:-1] if txt else txt; return s, s
                        backspace_btn.click(fn=_backspace, inputs=[text_state], outputs=[detected_text, text_state])
            ...
            with gr.Tab("📖 Reference"):
                # Pre-define the image component but don't render it yet
                # This fixes the UnboundLocalError by ensuring preview_img exists
                # before the buttons reference it in their click functions.
                ...
                with gr.Row():
                    # Left — Label Grid
                    with gr.Column(scale=2):
                        gr.Markdown("### 🔠 Sign Labels")
                        gr.Markdown("_Hover or click a letter to see the sign image._")
                        
                        # Use a compact grid for labels
                        for row_start in range(0, len(ALL_SIGNS), 6):
                            chunk = ALL_SIGNS[row_start : row_start + 6]
                            with gr.Row():
                                for sign in chunk:
                                    btn = gr.Button(
                                        sign, 
                                        variant="secondary",
                                        min_width=60,
                                        elem_classes="sign-label-btn"
                                    )
                                    
                                    # Function to find and return the image path
                                    def _make_get_path(s):
                                        def _get_path():
                                            p_png = os.path.join(SIGNS_DIR, f"{s}.png")
                                            p_jpg = os.path.join(SIGNS_DIR, f"{s}.jpg")
                                            return p_png if os.path.exists(p_png) else p_jpg
                                        return _get_path
                                    
                                    btn.click(fn=_make_get_path(sign), outputs=preview_img)

                    # Right — Preview Image
                    with gr.Column(scale=3):
                        gr.Markdown("### 🔍 Sign Preview")
                        # Now render the component we defined earlier
                        preview_img.render()
                        gr.Markdown(
                            "The preview updates automatically on hover."
                        )
            ...
            with gr.Tab("ℹ️ About"):
                with gr.Group():
                    gr.Markdown(
                        "## 🤟 AI Sign Language → Text Converter\n\n"
                        "---\n\n"
                        "This project detects **Indian Sign Language (ISL)** gestures using AI.\n\n"
                        "- ✅ Supports **A–Z** and **0–9** hand signs\n"
                        "- 🖐️ Uses **MediaPipe** 3D dual-hand landmark detection\n"
                        "- 🧠 Powered by a **Neural Network** (316-dim feature feed-forward model)\n"
                        "- ⏱️ Hold a sign steady for **2 seconds** at ≥70% confidence to confirm\n"
                        "- 📷 Works with live webcam stream or uploaded images\n\n"
                        "---\n\n"
                        "_Model: StaticGestureNet — 3D Dual-Hand + Bone Geometry + Landmark Cutout_"
                    )


    # ── CORS — allow HTML frontend on any origin ──────────────────────
    ...
    # ── Serve HTML / CSS / JS / assets at /ui ────────────────────────
    _ui_dir = os.path.dirname(os.path.abspath(__file__))
    demo.app.mount("/ui", _StaticFiles(directory=_ui_dir, html=True), name="html_ui")

    # ── Custom REST endpoint for HTML frontend ────────────────────────
    @demo.app.post("/predict_api")
    async def predict_api(request: _Request):
        try:
            ...
            return _JSONResponse({"error": f"Bad image data: {e}"}, status_code=400)

        result = predict(pil_img)
        if not result:
            return _JSONResponse({"error": "No result"})

        # Check for error/no-hand messages
        keys = list(result.keys())
        if keys and (keys[0].startswith("Error") or keys[0].startswith("No hand")):
            return _JSONResponse({"error": keys[0]})

        top5 = sorted(result.items(), key=lambda kv: -kv[1])
        letter, confidence = top5[0]
    ...
    print("\n[UI] ✅ HTML frontend  → http://127.0.0.1:7865/ui/")
    print("[UI]    Gradio backend  → http://127.0.0.1:7865/")
    print("[UI]    REST API        → http://127.0.0.1:7865/predict_api\n")
    demo.launch(server_name="127.0.0.1", server_port=7865, css=_FOOTER_CSS, js=_HOVER_JS)
